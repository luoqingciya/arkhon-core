...
// OHOS（OpenHarmony）沙箱对 VPN 隧道 fd 调用 Fstat 会返回 EPERM（受限 syscall），
// 而 gvisor fdbased 创建端点时必先 IsSocketFD → attach 直接失败，导致 gvisor 栈无法使用。
// 改为 getsockopt(SO_TYPE) 探测：真 socket 放行；非 socket / 受限环境容忍为 false，
// createInboundDispatcher 会退到 readv 模式（对 tap/tun/普通 fd 通用），不依赖 Fstat。
func IsSocketFD(fd int) (bool, error) {
	typ, err := unix.GetsockoptInt(fd, unix.SOL_SOCKET, unix.SO_TYPE)
	if err != nil {
		// 受限环境（如 OHOS 沙箱）或非 socket fd：容忍为 false，走 readv 通用 dispatcher。
		log.Printf("fdbased.IsSocketFD: fd=%d SO_TYPE probe error (%v); degraded to non-socket/readv path", fd, err)
		return false, nil
	}
	if typ <= 0 {
		log.Printf("fdbased.IsSocketFD: fd=%d SO_TYPE=%d (non-socket); using readv dispatcher", fd, typ)
	}
	return typ > 0, nil
}
